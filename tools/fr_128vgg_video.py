...
#                print bbox
            ...
            octrx = nctrx
            octry = nctry
            nctrx = pred_boxes[0] + w/2
            nctry = pred_boxes[1] + h/2
            if nctrx - SHIFT < octrx:
                nctrx -= SHIFT
            elif nctrx + SHIFT > octrx:
                nctrx += SHIFT
            if nctry - SHIFT < octry:
                nctry -=SHIFT
            elif nctry  + SHIFT >octry:
                nctry +=SHIFT
            nw =  1.4*w
#            nh =  nw
            nh =  1.4*h
            nx1 = max(0, nctrx-nw/2)
            ny1 = max(0, nctry-nh/2)
            nx2 = min(im.shape[1], nctrx+nw/2)
            ny2 = min(im.shape[0], nctry+nh/2)
