...
detPicDir = toBaseDir + "detPics/"
if not os.path.isdir(detPicDir):
    os.makedirs(detPicDir)
clsPicDir = toBaseDir + "clsPics/"
...
if __name__ == '__main__':
    ...
    print 'Loaded network {:s}'.format(det_model)
...
#            cv2.imwrite(savePicDir+picName, frame)
            ...
            # cv2.waitKey(1)
                iou, gt_idx = IOU_multi(handbox[i],box_gt)
                if ( iou > iouP):
                    nTP += 1
                    ...
                    if small_h!=clsInputSize_ or small_w!=clsInputSize_:
                        ...
                        if not gesturelists[cls] == handcls_gt[gt_idx][0]:
                            print gt_idx, nhand, cls,handcls_gt
...
gtTotal = 0
tpTotal = 0
for gname in sorted(tp_dict):
    if gname == 'bg' or gname == 'bainian' or gname == 'baoquan':
        continue
    print "%12s"%(gname)," Recall:%.2f"%( float(tp_dict[gname])/float(gt_dict[gname]+1) ), \
        " Precision:%.2f"%( float(tp_dict[gname])/float(re_dict[gname]+0.00001) ), \
        " gt:%4d"%(gt_dict[gname]), " re:%4d"%(re_dict[gname]), " tp:%4d"%(tp_dict[gname])
    reTotal += re_dict[gname]
    gtTotal += gt_dict[gname]
    tpTotal += tp_dict[gname]
print "gesture recall:%.2f"%(float(tpTotal)/float(gtTotal)), "gesture precision:%.2f"%(float(tpTotal)/float(reTotal))
print "-------------------------------------------------"
print "hand_gt:", n_pos_gt, "hand_re:", n_pos_re, "hand_tp:", n_pos_tp
print "hand_recall: ", float(n_pos_tp)/float(n_pos_gt), "hand_precision:" , float(n_pos_tp)/float(n_pos_re)
